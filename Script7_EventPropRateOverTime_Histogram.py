...
'''
This Script creates a histogram of event propagations along faults in meters per hour over time
'''
import numpy as np
...
#Keranen Jones zone 35.4-35.8, (-97.5)-(-96.75)
    ...
    #Relocated easyQuake Catalog_2010 
...
catdf = catdf_eQuake_RelocNarrow
...
for i in range(0,len(catdfr.index)):
    rutc.append(UTCDateTime(int(catdfr.iloc[i,11]),int(catdfr.iloc[i,12]),
                            int(catdfr.iloc[i,13]),int(catdfr.iloc[i,14]),int(catdfr.iloc[i,15]),catdfr.iloc[i,16]).datetime)
catdfr['rutc'] = rutc
...
for z in range(len(list(unique_labels)[:-1])):
    ...
    res = stats.linregress(d_along, dtime) #meters and hours
    ...
    save_slopes.append(1/res.slope)
plt.hist(save_slopes,500)
plt.xlabel('m/hr')
plt.xlim(-0.6,0.6)
plt.title('Propagation Rates Along Faults')
    #plt.xlim(0,10)
